...
function PayDialog({
  ...
  // both controls come off the form rather than asking for answers that would
  // be ignored.
  ...
  const onSubmit = async (values: PaymentValues) => {
    ...
    const ok = await confirm({
      ...
      description: values.disbursementId
        ? `${formatCedis(Number(values.amountGhs))} on ${subject}, booked against a send the system already made. The money has already left the payout wallet, so nothing is deducted again; this records what it paid for. Only a reversal takes it back off.`
        : `${formatCedis(Number(values.amountGhs))} paid on ${subject} by ${
