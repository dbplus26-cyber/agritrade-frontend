...
export function RuleBadge({ action }: { action: ApprovalAction }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 rounded-[20px] px-[9px] py-[3px] text-[11.5px] leading-[1.4] font-[550] whitespace-nowrap"
...
export function OverageMeter({
  ...
  return (
    <div
      ...
      aria-label={sentence}
      className={cn(
        "flex h-1 w-full max-w-[150px] overflow-hidden rounded-[3px] bg-[var(--ap-track)]",
        className,
      )}
    >
      <span
...
export const apButton =
  "inline-flex items-center justify-center rounded-[7px] border px-[13px] py-1.5 text-[12.5px] leading-[1.4] font-[550] whitespace-nowrap cursor-pointer " +
...
export function SelfDecidedChip() {
  return (
    <span className="rounded-[5px] bg-[var(--ap-chip)] px-[7px] py-0.5 text-[11px] leading-[1.4] font-semibold text-[var(--ap-muted)]">
      Self approved
    </span>
  );
}
